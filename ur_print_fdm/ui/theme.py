...
def get_dark_theme() -> str:
    """应用现代深色工程主题"""
    return _dark_theme(DARK)


def get_light_theme() -> str:
    """浅色主题（白色系）"""
    return _dark_theme(LIGHT)


def apply_app_theme(use_dark: bool):
    """将主题应用到 QApplication，确保 Tooltip 等全局控件样式统一"""
    global _CURRENT_TOKENS, _CURRENT_MODE
    _CURRENT_TOKENS = DARK if use_dark else LIGHT
    _CURRENT_MODE = "dark" if use_dark else "light"

    from PyQt6.QtWidgets import QApplication
    app = QApplication.instance()
    if app is not None:
        app.setProperty("ui_theme", _CURRENT_MODE)
        app.setStyleSheet(get_dark_theme() if use_dark else get_light_theme())


# Keep a lightweight runtime pointer for theme-aware helpers (icons, rich text, etc.)
